...
function manage_storage() {
	...
	const addRow = (k, imgSrc) => {
		...
		const href = `#${k.replace("image#", "local:")}`;
                // Some embedded contexts behave best when links reuse the current window.
                // The Discord Activity can open external links (with a prompt), but opening internally seems better,
        ...
        const target = is_discord_embed ? "_self" : "_blank";
