 ...
 * @param {string} [props.defaultFamily]
 * @param {number} [props.defaultSize]
 * @param {{ bold?: boolean; italic?: boolean; underline?: boolean; vertical?: boolean }} [props.defaultFormatting]
 * @param {(state: { family: string; size: number; bold: boolean; italic: boolean; underline: boolean; vertical: boolean }) => void} [props.onChange]
...
export function FontBox({
        ...
        const toggle = useCallback((setter) => {
                setter((prev) => !prev);
        }, []);

        return (
                <Component title="Fonts" className="font-box-component" orientation="tall" allowUndock>
                        <form className="font-box" aria-label="Font settings" onSubmit={(event) => event.preventDefault()}>
                                ...
                                <p id="font-family-status" className="font-status" aria-live="polite">
                                        {loadingFonts ? "Loading fonts…" : fontError ?? `Using ${availableFonts.length} font${availableFonts.length === 1 ? "" : "s"}.`}
                                </p>
                        </form>
                </Component>
        );
